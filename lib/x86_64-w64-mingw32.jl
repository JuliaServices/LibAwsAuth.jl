...
function Base.propertynames(x::aws_imds_iam_profile, private::Bool = false)
    (:last_updated, :instance_profile_arn, :instance_profile_id, if private
            fieldnames(typeof(x))
        else
            ()
        end...)
...
function Base.propertynames(x::aws_imds_instance_info, private::Bool = false)
    (:marketplace_product_codes, :availability_zone, :private_ip, :version, :instance_id, :billing_products, :instance_type, :account_id, :image_id, :pending_time, :architecture, :kernel_id, :ramdisk_id, :region, if private
            fieldnames(typeof(x))
        else
            ()
        end...)
...
function Base.getproperty(x::Ptr{aws_signing_config_aws}, f::Symbol)
    ...
    f === :flags && return Ptr{__JL_Ctag_26}(x + 160)
